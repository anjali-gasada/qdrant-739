...
## 4. Experiment C — chaos: kill a node mid-write (≈ 20 minutes)- doing
...
## 5. Experiment D — chaos: network partition (≈ 20 minutes) - done
...
## 6. Experiment E — segment size sweep (≈ 45 minutes) -- done
...
## 7. Experiment F — pipeline bottleneck localization (≈ 30 minutes)  -- done
...
# A3: single-node (network baseline)
./bin/bench \
  -hosts localhost:6334 \
  -collection sift1m \
  -workload A -concurrency 25 -duration 60s \
  -run-label pipeline_singlenode
```
...
## 8. Experiment G — batch size sweep (≈ 45 minutes) -done
...
for B in 64 256 512 1024 2048 4096; do
    ...
    # Delete collection from previous iteration
    ...
    ./bin/loader \
      ...
      -shard-number 6 -replication-factor 2 -write-consistency 1 \
      -batch $B -concurrency 8 || true

    # Force indexing and wait for it to complete
    curl -sf -X PATCH http://localhost:6333/collections/sift1m \
      -H 'Content-Type: application/json' \
      -d '{"optimizers_config": {"indexing_threshold_kb": 1000}}' >/dev/null

    echo "Waiting for indexing..."
    until [ "$(curl -s http://localhost:6333/collections/sift1m | \
      python3 -c "import sys,json; d=json.load(sys.stdin); \
      print(d['result']['indexed_vectors_count'] >= d['result']['points_count'])" 2>/dev/null)" = "True" ]; do
      sleep 10
    done
    echo "Indexing complete"

    # Bench writes only (workload A = 50% read / 50% update)
    ./bin/bench \
      -hosts localhost:6334,localhost:6434,localhost:6534 \
